...
loginUtil.signUp = function (user, callback) {
    if (!user.id || !user.password) {
        callback(util.convertToResponse({err: 'either username or passsword is missing'}, null, 'user ya password to de bhosdk'));
    }
    uidUtil.getUID(user.id, function (err, result) {
        if (result) {
            var id = result.uid;
            var fid = user.fid;
                ...
                if (!result.data) {
                    async.parallel({
                        one: function (callback) {
                            userUtil.createUser(id, fid, name, image, [], [], callback)
                        },
            ...
            callback(util.convertToResponse(err, result, 'Error occured while getting UID'))
        }
    });
};

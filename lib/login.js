...
loginUtil.forgotPassword = function (id, callback) {
    userUtil.getUser(id, function (res) {
        if (res.data) {
            ...
            if (!res) {
                callback({err: 'User is not registered with us'}, null, {err: 'User is not registered with us'});
            } else {
                callback({err: 'something broke please try again later'}, null, {err: 'something broke please try again later'});
            }
        }
    })

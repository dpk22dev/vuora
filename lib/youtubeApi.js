...
exports.createBroadcast = function ( data, callback ) {
    ...
    youtube.liveBroadcasts.insert(broadcastParams,  function(err,broadcast) {
        if (err) {
            ...
            data.error = {};
            data.error.msg = 'Error creating broadcast';
            data.error.err = err;
...
exports.getStreamStatus = function ( data, callback ) {
    ...
    youtube.liveStreams.list( streamParams, function ( err, result ) {
        if (err) {
            console.log('Error listing stream: ', err);
            data.error = {};
            data.error.msg = 'Error listing stream';
            data.error.err = err;

            callback( err, data );
        }
        var data = {};
        //considering 0th stream is ours
...
exports.setTransition = function ( data, callback ) {
    ...
    youtube.liveBroadcasts.transition( transitionParams, function ( err, result ) {
        if (err) {
            console.log('Error transitioning stream: ', err);
            data.error = {};
            data.error.msg = 'Error transitioning stream';
            data.error.err = err;
...
exports.searchVideos = function ( searchParams, callback ) {
    youtube.search.list(searchParams, function (err, response ) {
        var vidData ={};
        if (err) {
            console.log('Error getting videos for given tags: ', err);
            vidData.error = {};
            vidData.error.msg = 'Error getting videos for given tags';
            vidData.error.err = err;

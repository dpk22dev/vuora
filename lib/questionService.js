    ...
    questionCollection.findOne({_id: id}, function (err, result) {
        if (err) {
            ...
            setVoteCountForQuestion(result, function (err, result) {
                callback(util.convertToResponse(err, result, 'Error occured while getting vote from mongo'));
            })
        }
    });
};
        ...
        if (result) {
            ...
            if (to < new Date().getTime()) {
                ...
                questionCollection.insertOne(question, function (err, result) {
                    if (result) {
                        updateToElastic(question._id, ES_QUESTION_TYPE, function (err, result) {
                            callback(util.convertToResponse(err, {qId: question._id}, "Error occured while inserting to elastic"));
                        })
                    } else {
                        callback(util.convertToResponse(err, {qId: question._id}, "Error occured while inserting to mongo"));
                    }
                })
            }
        } else {
...
question.vote = function (data, callback) {
    ...
    var mongoDB = mongo.getInstance();
    var questionCollection = mongoDB.collection(QUESTION_VOTE);
    questionCollection.findOne({qId: id, user: data.user, vote: qVote}, function (err, result) {
        if (!result) {
            var qVoteObj = new QVote(id, qVote, data.user);
            questionCollection.insertOne(qVoteObj, function (err, res) {
                if (res) {
                    updateToElastic(id, QUESTION_VOTE, function (err, res) {
                        callback(util.convertToResponse(err, {data: 'Successfully inserted'}, 'Error occured while connecting to ES'))
                    })
                } else {
                    callback(util.convertToResponse(err, {data: 'Successfully inserted'}, 'Error occured while connecting to mongo'))
                }

            })
        } else {
            callback(util.convertToResponse({err: 'already done'}, null, 'dude you already provided same vote'));
        }
    });

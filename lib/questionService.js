...
question.getQuestionByTag = function (data, callback) {
    ...
    var mongoDB = mongo.getInstance();
    var questionCollection = mongoDB.collection(ES_QUESTION_TYPE);
    questionCollection.find({tags: {$in: tags}}).sort({createdAt: -1}).skip(page).limit(limit).toArray(function (err, results) {
        var ids = [];
        if (results) {
            results.forEach(function (result) {
                ids.push(result._id);
            });
            async.map(ids, question.getQuestion, function (err, results) {
                callback(utils.convertToResponse(err, results, 'Error occured while getting vote counts'));
            })
        } else {
            callback(utils.convertToResponse(err, null, 'Error occured while getting questions from DB'));
        }
    })
};

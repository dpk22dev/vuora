...
function Question(qId, tags, videoId, question, user, answered, time) {
    this._id = qId;
    this.tags = tags || []
...
function getVoteCountForQuestion(qId, callback) {
    var count = 0;
    var data = {};
    data._id = qId;
    setVoteCountForQuestion(data, function (err, result) {
        if (result) {
...
question.updateQuestion = function (qId, videoId, callback) {
    var mongoDB = mongo.getInstance();
    var questionCollection = mongoDB.collection(ES_QUESTION_TYPE);
    questionCollection.updateOne({_id: qId}, {$set: {videoId: videoId}}, function (err, res) {
        if (res) {

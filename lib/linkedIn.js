...
exports.getProfile = function (code, callback) {
    ...
    var authTokenUrl = format(config.linkedin.oauth1uri, config.linkedin.redirectionuri, config.linkedin.clientid, config.linkedin.clientsecret, code, config.linkedin.state);
    http.get(authTokenUrl, function (err, res) {
        if (res) {
            var resData = JSON.parse(res) || {};
            var token = resData.access_token;
            lOAuthTokenUrl = format(config.linkedin.oauth2uri, token);

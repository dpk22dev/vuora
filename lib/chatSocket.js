...
const REDIS_MEMBERS_PREFIX = 'sem_members_';
    ...
    chatNsp.on('connection', function (socket) {
        ...
        */
        ...
        socket.on('adduser', function (user) {
            ...
            user.conf = REDIS_MEMBERS_PREFIX + user.conf;
...
*/
        ...
        socket.on( 'removeQuestion', function ( data ) {
            console.log( data );
            socket.broadcast.to(socket.room).emit('removeQuestion', data );
        } )
        socket.on( 'questionVoteUpdated', function ( data ) {
            console.log( data );
            socket.broadcast.to(socket.room).emit('questionVoteUpdated', data );
        } )
    });

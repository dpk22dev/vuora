...
function getId(uid, callback) {
    redisClient.hget(UID_MAP, uid, function (err, data) {
        if (data) {
            callback(null, createResponse(data, uid));
        } else {
            callback({err: 'key not present'}, createResponse(uid, data));
        }
    })
}
...
uidUtil.getIDArray = function (uids, callback) {
    var obj = {};
    async.map(uids, getId, function (err, results) {
        results.forEach(function (result) {
            obj[result.uid] = result.id;
        });
        callback(err, obj);
    })
};

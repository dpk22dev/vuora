...
function updateToElastic(id, callback) {
    ...
    meeting.getEvent(id, function (result) {
        if (result.data) {
            esUtil.update(ES_INDEX, ES_EVENT_TYPE, id, result.data, function (err, result) {
                callback(err, result);
            });
        } else {
...
function requestEvent(data, mType, callback) {
    ...
    async.map(users, getUser, function (results) {
        var getUserRes = results.data;
        var okToCreate = true;
        if (getUserRes) {
            getUserRes.forEach(function (result) {
                if (!result) {
                    okToCreate = false;
                }
            });
        ...
        if (event) {
            ...
            seminarModel.createF2f({userId: user}, function (err, result) {
                if (result) {
                    ...
                    callback(util.convertToResponse({err: 'error occured in creating f2f'}, null, 'error occured in creating f2f'));
                }
            })
        } else {
                            ...
                            async.map(results, function (result, callback) {
                                ...
                                if (err) {
                                    ...
                                    async.map(ids, updateToElastic, function (err, result) {
                                        callback(util.convertToResponse(err, result, ''));
                                    });
                                }
                            });
                        }
